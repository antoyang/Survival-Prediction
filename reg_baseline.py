...
radiomics = pd.read_csv("data/train/features/radiomics.csv", index_col=0, header=[0,1,2])
print(radiomics)
clinical = pd.read_csv("data/train/features/clinical_data.csv", index_col=0, usecols=[3, 4])
print(clinical.head())
input_train = pd.concat([radiomics, clinical], axis=1)
print(input_train.head())
X_data = input_train.to_numpy()
X_data = preprocessing.scale(X_data)
output_train = pd.read_csv("data/train/y_train.csv", index_col=0, header=0)
Y_data = output_train.to_numpy()
print(Y_data)

X_train, X_test, Y_train, Y_test = model_selection.train_test_split(X_data, Y_data, test_size=0.21)
...
""" CoxPH Regression """
strct_Y_train = Surv.from_arrays(Y_train[:, 1].astype('bool'), Y_train[:, 0])
strct_Y_test = Surv.from_arrays(Y_test[:, 1].astype('bool'), Y_test[:, 0])
print(strct_Y_train)
coxph_reg = CoxPHSurvivalAnalysis(alpha=1e3)
coxph_reg.fit(X_train, strct_Y_train)
# prediction = pd.DataFrame()
print(coxph_reg.predict(X_test))
print(coxph_reg.score(X_test, strct_Y_test))
